  ...
  try {
    ...
    if ((!tickerResult.ticker || tickerFollowUp) && intentRoutingEnabled) {
      ...
      try {
        ...
        // Fix #1 (2026-06-03) — proactive market-wide fallback. A broad
        // no-ticker question that would otherwise hit the legacy greeting is
        // routed through the orchestrator's empty-overview plan (three
        // trending leaderboards). Gated by ORCA_PROACTIVE_FALLBACK (default
        // ON; set to 'false' for instant rollback to the greeting).
        const proactiveFallbackEnabled =
          process.env.ORCA_PROACTIVE_FALLBACK !== 'false'
        const runMarketWide =
          stageARoute.kind === 'market_wide' && proactiveFallbackEnabled
        if (runMarketWide) {
          decision = {
            ...decision,
            intent: 'overview',
        ...
        if (stageARoute.kind === 'orchestrator' || runMarketWide) {
          ...
          // EXCEPT the market-wide overview fallback (Fix #1) which routes a
          // broad no-ticker question here instead of the legacy greeting.
