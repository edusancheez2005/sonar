...
export const TOOL_CATALOGUE: ToolSpec[] = [
  ...
  { name: 'getWhaleFlows', purpose: 'Net whale buy/sell flow for ONE ticker over a window, PLUS the biggest individual buy/sell transactions (labeled wallets) — use this to answer "who were the biggest buyers/sellers of <token>?".', args: { ticker: 'string', window: '"1h"|"4h"|"24h"|"7d"|"30d"' }, cost: 'medium' },
  { name: 'getTrendingWhales', purpose: 'Market-wide PER-TOKEN whale-flow leaderboard (net buy/sell by token; no ticker needed). Optional chain filter ("solana whales only").', args: { window: '"1h"|"4h"|"24h"|"7d"|"30d"', chain: '"ethereum"|"solana"|"bsc"|"polygon" (optional)' }, marketWide: true, cost: 'medium' },
  { name: 'getLargestTransactions', purpose: 'Market-wide LARGEST INDIVIDUAL whale transactions (raw transfers, labeled wallets) — use for "biggest whale transactions/moves today", "largest transfers this week", optionally per chain.', args: { window: '"1h"|"4h"|"24h"|"7d"|"30d"', chain: '"ethereum"|"solana"|"bsc"|"polygon" (optional)', limit: 'number, default 10' }, marketWide: true, cost: 'medium' },
  ...
  { name: 'getMostActiveWallets', purpose: 'Ranked most-active wallets by TRANSACTION COUNT (no ticker needed). Activity, not profit — for profitability use getTopPerformingWallets.', args: { window: '"1h"|"4h"|"24h"|"7d"|"30d"' }, marketWide: true, cost: 'medium' },
