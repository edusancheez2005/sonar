...
- If \`getTopPerformingWallets\` returned data, present a ranked markdown TABLE of the most profitable wallets. Columns: Rank | Wallet | 7d return | Trades | Smart-money score. Show \`label\` next to \`address_short\` when present. Lead with the top performer's return. ALWAYS add one sentence explaining the methodology field (simulated replay of the wallet's real trades from a $10k stake) so the percentages aren't mistaken for raw balance changes.
- LIVE FEED: when \`transfer_feed.source\` is \`etherscan_live\`, say the activity was pulled live from Ethereum, report transfer counts / directions / tokens touched and the latest transfers by token amount, and note that USD values aren't available for these. When \`resolved_from_short\` is present, mention you resolved the shortened address to the full one.
