...
Buy/Sell Ratio: ${context.whales.buy_sell_ratio} ${context.whales.buy_count > context.whales.sell_count ? '[MORE INFLOW TX]' : context.whales.sell_count > context.whales.buy_count ? '[MORE OUTFLOW TX]' : '[BALANCED]'}
...
INTERPRETATION:
${context.whales.net_flow_24h > 0 ? `Positive net flow of ${formatCurrency(context.whales.net_flow_24h)} (descriptive observation — net inflow into whale wallets)` : context.whales.net_flow_24h < 0 ? `Negative net flow of ${formatCurrency(Math.abs(context.whales.net_flow_24h))} (descriptive observation — net outflow from whale wallets)` : 'Net flow is balanced'}
${context.whales.buy_count > context.whales.sell_count * 1.5 ? 'High inflow share observed in the window (descriptive only).' : context.whales.sell_count > context.whales.buy_count * 1.5 ? 'High outflow share observed in the window (descriptive only).' : 'Inflow/outflow activity relatively balanced.'}
