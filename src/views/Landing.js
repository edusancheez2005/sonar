...
// Lazy: only loads the wallet stack (~hundreds of KB) when the user actually
// clicks the wallet signup button, keeping landing first paint snappy.
const WalletSignupButton = dynamic(() => import('@/components/wallet/WalletSignupButton'), { ssr: false });
      ...
      {showLoginModal && (
        <Modal initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <FormContainer initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }}>
            ...
            <button type="button" onClick={async () => { if (isInAppBrowser()) { if (!escapeInAppBrowser()) setLoginError(IN_APP_BROWSER_MESSAGE); return; } try { const sb = supabaseBrowser(); const { error } = await sb.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/dashboard` } }); if (error) setLoginError('Google sign-in failed.'); } catch { setLoginError('An error occurred.'); } }}
        ...
        <Modal initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={e => { if (e.target === e.currentTarget) setShowSignupModal(false); }}>
          ...
          >
            ...
            <div style={{
              ...
              {/* Wallet sign-up — lazy-mounted (loads wagmi + RainbowKit on click only). */}
              <WalletSignupButton
                acceptedTerms={formData.acceptedTerms}
                onError={(msg) => { setSignupError(msg); showToast(msg, 'error'); }}
                onSignedIn={() => { showToast('Wallet linked. Welcome!', 'success'); setShowSignupModal(false); navigate('/dashboard'); }}
              />
